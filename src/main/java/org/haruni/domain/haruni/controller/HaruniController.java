...
public class HaruniController {
    ...
    @PostMapping("/chat")
    public ResponseEntity<ResponseDto<Chat>> processMessage(@AuthenticationPrincipal User user,
                                                            @Valid@RequestBody ChatRequestDto request){
        return ResponseEntity.status(HttpStatus.OK).body(ResponseDto.of(haruniService.processMessage(user, request), "채팅 처리 완료"));
    }
}
